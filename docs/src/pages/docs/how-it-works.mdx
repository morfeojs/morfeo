...
Even tho is missing most of the slices, the theme defined in the example is actually a valid theme.
This is because Morfeo comes with a default theme that is always merged with yours.
...
Also, Morfeo generates **Atomic CSS** bundled in a **single** and **small** file:

<BundleSize showText={false} />

The benefits of this choice are massive since there is **no duplication of CSS rules** and, most importantly, the CSS bundle does not grow with your application, and you'll not even notice since all of these happen behind the scenes.
...
```sh
concurrently \"morfeo extract\" \"npm run dev\"
```
